/**
Demo-set of animations using the DotStar painter library
...
https://github.com/DedeHai/DotStarPainter/issues
...
void setup() {
	...
	Serial.begin(115200);
	Serial.println(" ");
	Serial.println("DotStar Painter Demo");
...
void loop() {
	...
	for(loopcounter = 0; loopcounter<duration; loopcounter++) 
	{
		...
		pixelcanvas.transfer(); //transfer (add) the canvas to the pixels
	...
	for(loopcounter = 0; loopcounter<duration; loopcounter++) 
	{
		...
		{
			pixels.setPixelColor(i, 1, 0, 6); //color in RGB: dark blue
		}


		pixelbrush.paint(); //paint the brush to the canvas 
		pixelcanvas.transfer(); //transfer (add) the canvas to the DotStar
	...
	while(true) //create a loop with an additional brush (is deleted automatically once the loop finishes)
	{
		...
		DotStarPainterBrush pixelbrush2 = DotStarPainterBrush(&pixelcanvas); 
		...
		for(loopcounter = 0; loopcounter<duration; loopcounter++) 
		{
			...
			pixelcanvas.transfer(); //transfer the canvas to the pixels
	...
	while(true) //create a loop with two additional brushes (are deleted automatically once the loop finishes)
	{
		...
		DotStarPainterBrush pixelbrush2 = DotStarPainterBrush(&pixelcanvas); 
		DotStarPainterBrush pixelbrush3 = DotStarPainterBrush(&pixelcanvas); 
		...
		for(loopcounter = 0; loopcounter<duration; loopcounter++) 
		{
			...
			pixelcanvas.transfer(); //transfer the canvas to the pixels (and update i.e. fade pixels)
	...
	while(true) //create a loop with two additional brushes (are deleted automatically once the loop finishes)
	{
		...
		DotStarPainterCanvas pixelcanvas2 = DotStarPainterCanvas(&pixels);

		//create additional brush, painting on the second canvas 
		DotStarPainterBrush pixelbrush2 = DotStarPainterBrush(&pixelcanvas2); 
		...
		for(loopcounter = 0; loopcounter<duration; loopcounter++) 
		{
			...
			pixels.clear(); //remove any previously applied paint
			...
			pixelcanvas.transfer(); //transfer the first canvas to the dotstar
			pixelcanvas2.transfer(); //transfer the sedonc canvas to the dotstar pixels (adding colors, rather than overwriting colors)
